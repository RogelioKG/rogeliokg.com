...
#### `exports` 
  ```json
  {
    ...
    "exports": {
      ...
      ".": {
        "types": "./dist/index.d.ts",   // 如果使用者用 TypeScript，給他型別檔 (注意：types 必須放在最前面)
        "import": "./dist/index.mjs",   // 如果使用者寫 import pkg from 'my-package'，給他 ESM 版本
        "require": "./dist/index.cjs"   // 如果使用者寫 require('my-package')，給他 CommonJS 版本
      },
...
![wow-so-much-fun](https://i.imgur.com/lu8TTRc.png)
...
### `npm publish`
  ...
  **npm 的 private 發布是付費功能**
  <br>注意 [scoped package](#scoped-package) 預設是 private 發布，<br>如果免費帳號使用預設是會發布失敗的，要手動指定成 public 發布。
  ::
...
![project's node_modules](https://i.imgur.com/y75PZFz.png)
